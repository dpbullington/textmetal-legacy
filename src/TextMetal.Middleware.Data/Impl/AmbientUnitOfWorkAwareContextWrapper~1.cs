			...
			try
			{
				if (this.ShouldDisposeResources)
				{
					try
					{
						...
						context = this.SourceUnitOfWork.Context;

						if ((object)context == null)
							throw new InvalidOperationException(string.Format("Multicast context obtained from the current data source transaction context is null."));

						multicastDisposableContext = context as MulticastDisposableContext<TDisposableContext>;

						// will fail if not correct type (e.g. DataContext, DbContext, etc.)
						if ((object)multicastDisposableContext == null)
							throw new InvalidOperationException(string.Format("Multicast context type '{0}' obtained from the current data source transaction context does not match the current multicast context type '{1}'.", context.GetType().FullName, typeof(MulticastDisposableContext<TDisposableContext>).FullName));
