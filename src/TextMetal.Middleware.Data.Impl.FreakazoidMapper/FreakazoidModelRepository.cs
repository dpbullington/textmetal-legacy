	...
	public class FreakazoidModelRepository : ModelRepository<FreakazoidContext>
	{
		...
		public string DataSourceTag
		{
			get
			{
				...
				if (!AppConfigFascade.Instance.HasAppSetting(dataSourceTag))
					return null;

				value = AppConfigFascade.Instance.GetAppSetting<string>(dataSourceTag);
		...
		private bool DisableEnumerationReentrantCheck
		{
			get
			{
				...
				if (!AppConfigFascade.Instance.HasAppSetting(disableEnumerationReentrantCheck))
					return false;

				value = AppConfigFascade.Instance.GetAppSetting<bool>(disableEnumerationReentrantCheck);
		...
		public override TProjection Query<TProjection>(IUnitOfWork unitOfWork, Func<FreakazoidContext, TProjection> dataContextQueryCallback)
		{
			throw new NotSupportedException(string.Format("Query method is currently only useful in LINQ-based custom projection scnearios."));
		}
