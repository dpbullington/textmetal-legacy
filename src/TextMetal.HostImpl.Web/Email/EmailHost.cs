...
namespace TextMetal.HostImpl.Web.Email
{
	public sealed class EmailHost : IEmailHost
	{
		...
		public void Host(bool strictMatching, MessageTemplate messageTemplate, object modelObject, IHostEmailMessage hostEmailMessage)
		{
			...
			{
				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.Subject))
					mailMessage.Subject = hostEmailMessage.Subject;

				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.Body))
					mailMessage.Body = hostEmailMessage.Body;

				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.To))
				{
				...
				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.From))
					mailMessage.From = new MailAddress(hostEmailMessage.From);

				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.Sender))
					mailMessage.Sender = new MailAddress(hostEmailMessage.Sender);

				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.ReplyTo))
				{
				...
				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.BlindCarbonCopy))
				{
				...
				if (!DataType.IsNullOrWhiteSpace(hostEmailMessage.CarbonCopy))
				{
