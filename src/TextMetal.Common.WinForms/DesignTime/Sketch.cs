...
using TextMetal.Common.WinForms.DesignTime.Shapes;

namespace TextMetal.Common.WinForms.DesignTime
{
	[Serializable]
	...
	public class Sketch
	{
		...
		public static Sketch GetSketchFromXmlData(string xmlData)
		{
			...
			if (DataType.Instance.IsNullOrWhiteSpace(xmlData))
				sketch = new Sketch();
			else
			{
		...
		public static Sketch LoadFromFile(string filePath)
		{
			...
			if (DataType.Instance.IsNullOrWhiteSpace(filePath))
