...
{
	[XmlElementMapping(LocalName = "ForEach", NamespaceUri = "http://www.textmetal.com/api/v6.0.0", ChildElementModel = ChildElementModel.Sterile)]
	public sealed class ForEachConstruct : TemplateXmlObject
	{
		...
		protected override void CoreExpandTemplate(ITemplatingContext templatingContext)
		{
			...
			if (!LegacyInstanceAccessor.DataTypeFascadeLegacyInstance.IsNullOrWhiteSpace(varItem))
			{
				new AllocConstruct()
				{
					Token = varItem
			...
			if (!LegacyInstanceAccessor.DataTypeFascadeLegacyInstance.IsNullOrWhiteSpace(varCt))
			{
				new AllocConstruct()
				{
					Token = varCt
