...
#if __IfOnlyAmbientUnitOfWorkContext__
		protected TContext __IfOnlyAmbientUnitOfWorkContext__
		{
			get
			{
				TContext value = null;
				AmbientUnitOfWorkAwareContextWrapper<TContext> wrapper = null;

				if ((object)UnitOfWork.Current != null)
				{
					using (wrapper = this.GetContext(UnitOfWork.Current))
						value = wrapper.DisposableContext;

					if (!wrapper.IsUsableWrappedContext)
						value = null;
				}
