...
namespace TextMetal.HostImpl.AspNetSample.DomainModel.Views
{
	...
	{		
		...
		public static bool Exists(EventLogExtent eventLogExtent)
		{
			...
			eventLogExtents =
				Stuff.Get<IRepository>("").FindEventLogExtents(
					q =>
						q.Where(
							z =>
								(z.XXX == eventLogExtent.XXX) && ((object)eventLogExtent.EventLogExtentId == null || z.EventLogExtentId != eventLogExtent.EventLogExtentId)));

			return eventLogExtents.Count() > 0;
		}
		...
		public virtual Message[] Validate()
		{
			...
			if (exists)
				messages.Add(new Message("", "EventLogExtent must be unique.", Severity.Error));
