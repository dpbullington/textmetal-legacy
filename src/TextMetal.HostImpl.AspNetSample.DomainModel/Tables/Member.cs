...
namespace TextMetal.HostImpl.AspNetSample.DomainModel.Tables
{
	public partial class Member
	{
		...
		partial void OnValidate(ref IEnumerable<Message> messages)
		{
			...
			if (DataType.IsNullOrWhiteSpace(this.MemberName))
				_messages.Add(new Message("", "Member name is required.", Severity.Error));
			...
			if (exists)
				_messages.Add(new Message("", "Member must be unique.", Severity.Error));
