...
namespace TextMetal.HostImpl.AspNetSample.DomainModel.Tables
{
	public partial class User
	{
		...
		public static Message[] ValidateForAuthentication(string username, string password)
		{
		...
		public static Message[] ValidateForForgotPassword(string username)
		{
			List<Message> messages;
		...
		public static Message[] ValidateForForgotUserName(string emailAddress)
		{
			List<Message> messages;
		...
		public virtual Message[] Validate()
		{
			...
			if (DataType.IsNullOrWhiteSpace(this.UserName))
				...
				messages.Add(new Message("", "Username minimum length is 3.", Severity.Error));
			//else if (Regex.IsMatch(this.UserName, "[a-zA-Z][a-zA-Z0-9\\.-_]{1,255}", RegexOptions.IgnorePatternWhitespace))
			//messages.Add(new Message("", "Username is invalid.", Severity.Error));

			if (DataType.IsNullOrWhiteSpace(this.SaltValue))
				messages.Add(new Message("", "Salt value is required.", Severity.Error));
			...
			if (!DataType.IsNullOrWhiteSpace(this.TransientPassswordClearText) &&
				...
				messages.Add(new Message("", "Password minimum length is 4.", Severity.Error));

			if (DataType.IsNullOrWhiteSpace(this.Question))
				messages.Add(new Message("", "Security question is required.", Severity.Error));
			else if (this.Question.Length < 3)
				messages.Add(new Message("", "Security question minimum length is 3.", Severity.Error));

			if (DataType.IsNullOrWhiteSpace(this.AnswerHash))
				messages.Add(new Message("", "Security answer is required.", Severity.Error));

			if (!DataType.IsNullOrWhiteSpace(this.TransientAnswerClearText) &&
				this.TransientAnswerClearText.Length < 3)
				messages.Add(new Message("", "Security answer minimum length is 3.", Severity.Error));
			...
			if (exists)
				messages.Add(new Message("", "Username and email address each must be unique.", Severity.Error));
