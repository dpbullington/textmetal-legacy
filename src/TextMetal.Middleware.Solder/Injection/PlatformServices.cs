...
{
	internal sealed class AssemblyLoaderContainerContext
	{
		...
		private static readonly AssemblyLoaderContainerContext theOnlyAllowedInstance = new AssemblyLoaderContainerContext();
		...
		private static bool DisableResolutionAutoWire
		{
			get
			{
				...
				value = AppConfigFascade.Instance.GetAppSetting<bool>(String.Format("{0}::DisableResolutionAutoWire", typeof(AssemblyLoaderContainerContext).FullName));
