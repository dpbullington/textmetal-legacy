...
namespace TextMetal.Middleware.Solder.Interception.RemotingImpl
{
	/// <summary>
	/// Provides a (remoting) real proxy for objects which leverage the dynamic invoker infrastructure (i.e. IDynamicInvocation).
	/// </summary>
	public sealed class DynamicInvokerRealProxy : RealProxy, IDisposable, IRemotingTypeInfo
	{
		#region Constructors/Destructors

		/// <summary>
		/// Initializes a new instance of the DynamicInvokerRealProxy class.
		/// </summary>
		/// <param name="proxiedType"> The proxied type for which to return a proxy instance. </param>
		/// <param name="dynamicInvocation"> The dynamic invoker object to use. </param>
		public DynamicInvokerRealProxy(Type proxiedType, IDynamicInvocation dynamicInvocation)
			: base(proxiedType)
		{
		...
		public override IMessage Invoke(IMessage msg)
		{
...
#endif
			if (this.Disposed)
				throw new ObjectDisposedException(typeof(DynamicInvokerRealProxy).FullName);
			...
			returnValue = this.DynamicInvocation.Invoke(this.ProxiedType, invokedMethodInfo, proxyInstance, invocationParameters);
