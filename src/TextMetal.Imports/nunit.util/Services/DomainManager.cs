...
namespace NUnit.Util
{
	...
	public class DomainManager : IService
	{
		...
		private static Logger log = InternalTrace.GetLogger(typeof(DomainManager));
		...
		private class DomainUnloader
		{
			...
			public void Unload()
			{
				...
				try
				{
					domainName = this.domain.FriendlyName;
				}
				...
				this.thread = new Thread(new ThreadStart(this.UnloadOnThread));
				this.thread.Start();
			...
			private void UnloadOnThread()
			{
				...
				try
				{
					shadowCopy = this.domain.ShadowCopyFiles;
					cachePath = this.domain.SetupInformation.CachePath;
					domainName = this.domain.FriendlyName;
