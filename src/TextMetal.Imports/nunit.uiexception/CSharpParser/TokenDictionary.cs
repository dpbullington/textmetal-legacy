...
namespace NUnit.UiException.CodeFormatters
{
		...
		IEnumerable
	{
		...
		{
			this._list = new List<InternalLexToken>();
			this._working = new List<LexToken>();
		...
		public void Add(string value, LexerTag tag)
		{
			...
			if (this.Count > 0)
			{
				UiExceptionHelper.CheckTrue(
					this._list[this.Count - 1].Text.Length >= value.Length,
			...
			foreach (InternalLexToken item in this._list)
			{
				if (item.Text.StartsWith(value))
					newToken.StartingWith.Add(item);
			}

			this._list.Add(newToken);
		...
		{
			foreach (LexToken item in this._list)
			{
				if (item.Text == value)
					return (true);
			}
		...
		public LexToken TryMatch(string text, string prediction)
		{
			...
			foreach (InternalLexToken token in this._list)
			{
				if (text.EndsWith(token.Text))
				{
					...
					if (prediction != "")
					{
						...
						this._working.Clear();
						this.PopulateTokenStartingWith(token, this._working);
			...
			LexToken
		{
			...
			public InternalLexToken(string value, LexerTag tag)
			{
				...
				this.StartingWith = new List<LexToken>();
