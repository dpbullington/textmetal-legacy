	...
	public class ClassifiedTokenCollection
	{
		...
		public ClassifiedToken this[int index]
		{
			get
			{
				...
				result = (InternalToken)this._token;
				this._populateToken(this._lineIndex, index, result);
		...
		public string Text
		{
			get
			{
				...
				index_start_ptr = this._info.LineArray[this._lineIndex];
				index_start = this._info.IndexArray[index_start_ptr];

				if (this._lineIndex + 1 >= this._info.LineArray.Count)
					index_end = this._info.Text.Length;
				else
				{
					index_end_ptr = this._info.LineArray[this._lineIndex + 1];
					index_end = this._info.IndexArray[index_end_ptr];
				}
				...
				text = this._info.Text.Substring(index_start, index_end - index_start);
				text = text.TrimEnd();
		...
		private ClassificationTag _getTagFromByteValue(byte value)
		{
			switch (value)
			{
				...
				case 2:
					return (ClassificationTag.Comment);
				case 3:
					return (ClassificationTag.String);
				default:
		...
		private void _populateToken(int lineIndex, int tokenIndex, InternalToken output)
		{
			...
			tagZero = this._info.LineArray[lineIndex];
			tagStart = tagZero + tokenIndex;
			tagEnd = tagStart + 1;

			strIndex_start = this._info.IndexArray[tagStart];

			if (tagEnd < this._info.IndexArray.Count)
			{
				strIndex_end = this._info.IndexArray[tagEnd];
				res = this._info.Text.Substring(strIndex_start, strIndex_end - strIndex_start);
			}
			else
				res = this._info.Text.Substring(strIndex_start);
			...
			tag = this._getTagFromByteValue(this._info.TagArray[tagStart]);
