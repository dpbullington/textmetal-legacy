...
namespace Castle.DynamicProxy
{
	...
	public class ModuleScope
	{
		...
		public string StrongNamedModuleName
		{
			get
			{
				return Path.GetFileName(this.strongModulePath);
			}
		}
		...
		/// </summary>
		/// <value>
		/// The directory where the strongly named module generated by this scope will be saved when
		/// <see
		///     cref="SaveAssembly()" />
		/// is called
		...
		public string StrongNamedModuleDirectory
		{
			get
			{
				var directory = Path.GetDirectoryName(this.strongModulePath);
		...
		public string WeakNamedModuleName
		{
			get
			{
				return Path.GetFileName(this.weakModulePath);
			}
		}
		...
		/// </summary>
		/// <value>
		/// The directory where the weakly named module generated by this scope will be saved when
		/// <see
		///     cref="SaveAssembly()" />
		/// is called
		/// (if this scope was created to save modules).
		...
		public string WeakNamedModuleDirectory
		{
			get
			{
				var directory = Path.GetDirectoryName(this.weakModulePath);
		...
		/// <returns>
		/// A strong-named or weak-named module generated by this scope, as specified by the
		/// <paramref
		///     name="isStrongNamed" />
		/// parameter.
		/// </returns>
		...
		{
			var assemblyName = this.GetAssemblyName(signStrongName);
			var moduleName = signStrongName ? this.StrongNamedModuleName : this.WeakNamedModuleName;
#if !SILVERLIGHT
			if (this.savePhysicalAssembly)
			{
				...
				try
				{
					assemblyBuilder = AppDomain.CurrentDomain.DefineDynamicAssembly(
						assemblyName, AssemblyBuilderAccess.RunAndSave, signStrongName ? this.StrongNamedModuleDirectory : this.WeakNamedModuleDirectory);
				}
		...
		public string SaveAssembly()
		{
			...
			if (this.WeakNamedModule != null)
				return this.SaveAssembly(false);

			return null;
		}

		/// <summary>
		/// Saves the specified generated assembly with the name and directory information given when this
		/// <see
		///     cref="ModuleScope" />
		/// instance was created
		/// (or with the <see cref="DEFAULT_FILE_NAME" /> and current directory if none was given).
		/// </summary>
		/// <param name="strongNamed">
		/// True if the generated assembly with a strong name should be saved (see
		/// <see
		///     cref="StrongNamedModule" />
		/// );
		/// false if the generated assembly without a strong name should be saved (see <see cref="WeakNamedModule" />.
		/// </param>
		/// <remarks>
		///     <para>
		...
		/// No assembly has been generated that matches the
		/// <paramref
		///     name="strongNamed" />
		/// parameter.
		/// </exception>
		...
		public string SaveAssembly(bool strongNamed)
		{
			...
			if (strongNamed)
			{
				...
				assemblyBuilder = (AssemblyBuilder)this.StrongNamedModule.Assembly;
				assemblyFileName = this.StrongNamedModuleName;
				assemblyFilePath = this.StrongNamedModule.FullyQualifiedName;
			}
			else
			{
				if (this.WeakNamedModule == null)
					throw new InvalidOperationException("No weak-named assembly has been generated.");
				assemblyBuilder = (AssemblyBuilder)this.WeakNamedModule.Assembly;
				assemblyFileName = this.WeakNamedModuleName;
				assemblyFilePath = this.WeakNamedModule.FullyQualifiedName;
			}

			if (File.Exists(assemblyFilePath))
				File.Delete(assemblyFilePath);

			this.AddCacheMappings(assemblyBuilder);
		...
		private void AddCacheMappings(AssemblyBuilder builder)
		{
			...
			using (this.Lock.ForReading())
			{
				...
				foreach (var cacheEntry in this.typeCache)
				{
		...
		/// </summary>
		/// <param name="assembly">
		/// The assembly to load types from. This assembly must have been saved via
		/// <see
		///     cref="SaveAssembly(bool)" />
		/// or
		/// <see cref="SaveAssembly()" />, or it must have the <see cref="CacheMappingsAttribute" /> manually applied.
		/// </param>
		/// <remarks>
		...
		public void LoadAssemblyIntoCache(Assembly assembly)
		{
			...
			foreach (var mapping in cacheMappings[0].GetDeserializedMappings())
			{
				...
				if (loadedType != null)
					this.RegisterInCache(mapping.Key, loadedType);
			}
		}
#endif
		...
		{
			var module = this.ObtainDynamicModule(this.disableSignedModule == false && inSignedModulePreferably);
