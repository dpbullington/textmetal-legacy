...
namespace NUnit.Framework.Constraints
{
	...
	public abstract class PathConstraint : Constraint
	{
		...
		{
			return string.Format("<{0} \"{1}\" {2}>", this.DisplayName, this.expectedPath, this.caseInsensitive ? "ignorecase" : "respectcase");
		}
		...
		protected bool IsSamePathOrUnder(string path1, string path2)
		{
			...
			if (length1 == length2)
				return string.Compare(path1, path2, this.caseInsensitive) == 0;

			// path 2 is longer than path 1: see if initial parts match
			if (string.Compare(path1, path2.Substring(0, length1), this.caseInsensitive) != 0)
				return false;
		...
		public override bool Matches(object actual)
		{
			...
			return this.IsMatch(this.expectedPath, this.actualPath);
		}
	...
	public class SamePathConstraint : PathConstraint
	{
		...
		{
			return IsSamePath(Canonicalize(expectedPath), Canonicalize(actualPath), this.caseInsensitive);
		}
	...
	public class SubPathConstraint : PathConstraint
	{
		...
		protected override bool IsMatch(string expectedPath, string actualPath)
		{
			...
			return IsSubPath(Canonicalize(expectedPath), Canonicalize(actualPath), this.caseInsensitive);
		}
	...
	public class SamePathOrUnderConstraint : PathConstraint
	{
		...
		protected override bool IsMatch(string expectedPath, string actualPath)
		{
			...
			return IsSamePath(path1, path2, this.caseInsensitive) || IsSubPath(path1, path2, this.caseInsensitive);
		}
