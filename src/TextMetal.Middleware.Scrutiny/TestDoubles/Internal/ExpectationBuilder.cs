...
namespace NMock.Internal
{
	internal class ExpectationBuilder : IMethodSyntax, IArgumentSyntax, IMatchSyntax
	{
		...
		{
			this.BuildableExpectation = new BuildableExpectation(description, requiredCountMatcher, matchingCountMatcher);
		}
		...
		public void Assert()
		{
			this.BuildableExpectation.Assert();
		}
		...
		{
			this.BuildableExpectation.AddComment(comment);
			return this.BuildableExpectation;
		}
		...
		{
			var matches = this.MockObject.GetMethodsMatching(methodMatcher);
		...
		{
			Matcher methodMatcher = new DescriptionOverride(propertyName, new MethodNameMatcher(Constants.GET + propertyName, this.MockObject.MockedTypes.PrimaryType));

			this.EnsureMatchingMethodExistsOnMock(methodMatcher, "a getter for property " + propertyName);

			this.BuildableExpectation.MethodMatcher = methodMatcher;
			this.BuildableExpectation.ArgumentsMatcher = new DescriptionOverride(String.Empty, new ArgumentsMatcher());
		}
		...
		{
			this.BuildableExpectation.AddInvocationMatcher(matcher);
			return this;
		}
		...
		*/
		...
		protected IArgumentSyntax Method(MethodInfo methodInfo)
		{
			...
			return this.Method(new MethodMatcher(methodInfo));
		}
		...
		{
			this.MockObject = receiver;

			this.BuildableExpectation.Receiver = this.MockObject;
			this.MockObject.AddExpectation(this.BuildableExpectation);
		}
		...
		public IArgumentSyntax ProtectedMethod(string methodName)
		{
			...
			return this.Method(new MethodNameMatcher(methodName, this.MockObject.MockedTypes.PrimaryType));
		}

		protected IValueSyntaxBuilder SetIndexer(params object[] expectedArguments)
		{
			Matcher methodMatcher = new DescriptionOverride(String.Empty, new MethodNameMatcher(Constants.SET_ITEM, this.MockObject.MockedTypes.PrimaryType));
			this.EnsureMatchingMethodExistsOnMock(methodMatcher, "an indexed setter");

			this.BuildableExpectation.DescribeAsIndexer();
			this.BuildableExpectation.MethodMatcher = methodMatcher;
			return new IndexSetterBuilder(this.BuildableExpectation, this).WithParameters(expectedArguments);
		}
		...
		{
			Matcher methodMatcher = new DescriptionOverride(propertyName + " = ", new MethodNameMatcher(Constants.SET + propertyName, this.MockObject.MockedTypes.PrimaryType));
		...
		public ICommentSyntax Will(params IAction[] actions)
		{
			...
			if (this._method != null && this._method.ReturnType != typeof(void))
			{
				foreach (var action in actions)
				{
					var returnAction = action as IReturnAction;
					if (returnAction != null && !this._method.ReturnType.IsAssignableFrom(returnAction.ReturnType))
						throw new InvalidOperationException(String.Format("The return type '{0}' does not match the return type '{1}' on method {2}.", returnAction.ReturnType, this._method.ReturnType, this._method.Name));
				}
			}

			foreach (IAction action in actions)
				this.BuildableExpectation.AddAction(action);
			...
			if (this._method != null)
			{
				...
				for (int i = 0; i < expectedArguments.Length; i++)
				{
					...
					if (expectedArgument as Matcher == null)
					{
						...
						var isNullable = _type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>) || !_type.IsValueType;
							...
							)
						{
							...
							throw new InvalidOperationException(String.Format("An argument of type '{0}' at index {1} does not match the method definition of '{2}' at {1} of the {3} method.", typeName, i, type, this._method.Name));
						}
					}
				}
			}
			return this.With(ArgumentMatchers(expectedArguments));
		}

		protected IActionSyntax With(params Matcher[] argumentMatchers)
		{
			return this.With(new ArgumentsMatcher(argumentMatchers));
		}

		protected internal IMatchSyntax With(Matcher matcher)
		{
			if (this._method != null && this._method.GetParameters().Length == 0 && !this.UsingMethodWith)
				throw new ExpectationException("Using a 'With' clause on a method that accepts 0 parameters is not valid.");

			this.BuildableExpectation.ArgumentsMatcher = matcher;
			return this;
		}
		...
		{
			return this.With(new AlwaysMatcher(true, "(any arguments)"));
		}
		...
		{
			return this.With(new DescriptionOverride("(no arguments)", new ArgumentsMatcher()));
		}
		...
		private class IndexSetterBuilder : IValueSyntaxBuilder
		{
			...
			public IActionSyntax To(Matcher matcher)
			{
				...
				this._matchers[this._matchers.Length - 1] = matcher;

				//set up the display
				this._expectation.ArgumentsMatcher = new IndexSetterArgumentsMatcher(this._matchers);
				return this._builder;
			}

			public void Will(params IAction[] action)
			{
				this._builder.Will(action);
			}
			...
			public IValueSyntaxBuilder WithParameters(params object[] expectedArguments)
			{
				...
				this._matchers = new Matcher[indexerMatchers.Length + 1];

				//copy the 
				Array.Copy(indexerMatchers, this._matchers, indexerMatchers.Length);
				return this;
			}
		...
		private class PropertyValueBuilder : IValueSyntaxBuilder
		{
			...
			{
				return this._builder.With(new ArgumentsMatcher(valueMatcher));
			}
