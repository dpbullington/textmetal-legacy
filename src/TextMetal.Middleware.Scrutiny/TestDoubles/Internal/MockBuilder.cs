...
namespace NMock.Internal
{
	...
	internal class MockBuilder : IMockDefinitionSyntax
	{
		...
		private static void CheckInterfacesDoNotContainToStringMethodDeclaration(CompositeType mockedTypes)
		{
			...
			{
				var _type = method.DeclaringType.GetTypeInfo();

				if (_type.IsInterface && method.GetParameters().Length == 0)
		...
		{
			if (this._name == null)
				this._name = primaryType.DefaultNameFor();

			this.DiscoverInterfaces(primaryType);

			var compositeType = new CompositeType(primaryType, this._types.ToArray());

			var _type = compositeType.PrimaryType.GetTypeInfo();

			if (_type.IsInterface)
			{
				if (this._constructorArgs.Length > 0)
			...
			return mockObjectFactory.CreateMock(
				mockFactory,
				compositeType, this._name, this._mockStyle ?? MockStyle.Default, this._constructorArgs);
		}
		...
		public IMockDefinitionSyntax Implementing(params Type[] additionalTypesToMock)
		{
			...
			lock (this.locker)
			{
				this._types.AddRange(additionalTypesToMock);
			}
		...
		{
			if (this._constructorArgs != EmptyArgsArray)
