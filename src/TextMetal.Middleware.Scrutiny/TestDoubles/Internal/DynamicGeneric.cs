...
namespace NMock.Internal
{
	...
	internal class Dynamic<T> : DynamicObject
	{
		...
		public Dynamic(T instance)
		{
			...
			this._propertyBag = new Dictionary<string, object>(StringComparer.CurrentCulture);
		...
		public override bool TryGetMember(GetMemberBinder binder, out object result)
		{
			...
			if (value)
			{
				result = ((PropertyInfo)prop).GetValue(this._instance, null);
				value = true;
			}
			else
				value = this._propertyBag.TryGetValue(binder.Name, out result);
		...
		public override bool TryInvokeMember(InvokeMemberBinder binder, object[] args, out object result)
		{
			...
			result = method.Invoke(this._instance, args);
		...
		public override bool TrySetMember(SetMemberBinder binder, object value)
		{
			...
			if (foundProp)
				((PropertyInfo)prop).SetValue(this._instance, value, null);
			else
				this._propertyBag[binder.Name] = value;
