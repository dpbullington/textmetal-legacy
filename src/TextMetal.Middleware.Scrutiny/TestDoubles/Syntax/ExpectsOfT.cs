...
namespace NMock
{
	...
	/// access it by typing <c> mockObject.Expects </c>.
	/// <para>
	/// The <see cref="Expects{T}" /> class provides access to
	/// all other methods that help create expectations.
	/// </para>
	/// </remarks>
		...
		where T : class
	{
		...
		public IMethodSyntax<T> Any
		{
			get
			{
				return this.GetExpectationBuilder("any number of times", Is.Anything, Is.Anything, this._proxy);
			}
		}
		...
		public IMethodSyntax<T> No
		{
			get
			{
				return this.GetExpectationBuilder("never", Is.EqualTo(0), Is.EqualTo(0), this._proxy);
			}
		}
		...
		{
			return this.GetExpectationBuilder("at least " + count.Times(), Is.AtLeast(count), Is.Anything, this._proxy);
		}
		...
		{
			return this.GetExpectationBuilder("at most " + count.Times(), Is.Anything, Is.AtMost(count), this._proxy);
		}
		...
		{
			return this.GetExpectationBuilder(minCount + " to " + maxCount + " times", Is.AtLeast(minCount), Is.AtMost(maxCount), this._proxy);
		}
		...
		{
			return this.GetExpectationBuilder(count.Times(), Is.AtLeast(count), Is.AtMost(count), this._proxy);
		}
