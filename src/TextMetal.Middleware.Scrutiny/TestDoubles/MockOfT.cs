...
namespace NMock
{
		...
		where T : class
	{
		...
		/// </summary>
		/// <remarks>
		/// This would be considered to be the main property of a <see cref="Mock{T}" /> class.  This property
		/// provides access to all other expectation setup methods.
		/// </remarks>
		/// <example>
		/// In this example:
		/// <list type="bullet">
		...
		public Expects<T> Expects
		{
			get
			{
				return new Expects<T>(this._proxy);
			}
		}
		...
		public string Name
		{
			get
			{
				return ((IMockObject)this._proxy).MockName;
			}
		}
		...
		public bool IgnoreUnexpectedInvocations
		{
			get
			{
				return ((IMockObject)this._proxy).IgnoreUnexpectedInvocations;
			}
			set
			{
				((IMockObject)this._proxy).IgnoreUnexpectedInvocations = value;
			}
		}
		...
		/// </summary>
		public bool ThrowToStringException
		{
			get;
			set;
		}
		...
		{
			return new ExpectationBuilder<T>("1 time", Is.AtLeast(1), Is.AtMost(1), this._proxy).Parse(expression);
		}
		...
		public Mock<TType> As<TType>(bool throwException) where TType : class
		{
			...
			TType mock = this._proxy as TType;
			if (mock != null)
				return new Mock<TType>(this._proxy, this._factory);
			else
			{
				if (throwException)
		...
		{
			this._factory.AssertAll(this._proxy as IMockObject);
		}
		...
		{
			this._factory.ClearExpectations(this._proxy as IMockObject);
		}
		...
		{
			if (this.ThrowToStringException)
