...
namespace Castle.DynamicProxy.Contributors
{
	...
	public class InvocationWithGenericDelegateContributor : IInvocationCreationContributor
	{
		...
		private Reference GetDelegate(AbstractTypeEmitter invocation, MethodEmitter invokeMethodOnTarget)
		{
			...
			var closedDelegateType = this.delegateType.MakeGenericType(genericTypeParameters);
			var localReference = invokeMethodOnTarget.CodeBuilder.DeclareLocal(closedDelegateType);
			var closedMethodOnTarget = this.method.MethodOnTarget.MakeGenericMethod(genericTypeParameters);
#else
			...
			invokeMethodOnTarget.CodeBuilder.AddStatement(this.SetDelegate(localReference, localTarget, closedDelegateType, closedMethodOnTarget));
