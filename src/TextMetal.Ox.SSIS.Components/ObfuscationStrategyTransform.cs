...
using TextMetal.Middleware.Oxymoron.Legacy;
...
namespace TextMetal.Ox.SSIS.Components
{
	...
	public class ObfuscationStrategyTransform : PipelineComponent, IOxymoronHost
	{
		...
		private DbConnection dictionaryDbConnection;
		...
		public override void PreExecute()
		{
			...
			foreach (DictionaryConfiguration dictionaryConfiguration in obfuscationConfiguration.DictionaryConfigurations)
			{
				...
				dictionaryAdapter.InitializePreloadCache((r) => r, dictionaryConfiguration, this.OxymoronEngine.SubstitutionCacheRoot);
		...
		/// </summary>
		/// <returns> </returns>
		private bool TryGetDictionaryDbConnection(bool open, out DbConnection dbConnection)
		{
			IDTSRuntimeConnection100 dtsRuntimeConnection100;
			...
			if ((object)dtsRuntimeConnection100.ConnectionManager != null)
			{
				...
				if (open)
					dbConnection = (DbConnection)dtsConnectionManagerDatabaseParameters100.GetConnectionForSchema();
			}
