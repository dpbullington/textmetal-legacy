...
using LeastViable.Common.BasicUtility;
using LeastViable.VsIdeConv.ConsoleTool.Utilities;
	...
	public class VsSolutionFileHandler : FileHandler
	{
		...
		public static bool PatchSolutionFile(FileInfo fileInfo, ref string[] solutionLines)
		{
			...
			for (int index = 0; index < solutionLines.Length; index++)
			{
				...
				{
					line = !DataType.Instance.IsNullOrWhiteSpace(ConversionConfig.ConversionSettings.SolutionInternalVersion) ?
				...
				{
					line = !DataType.Instance.IsNullOrWhiteSpace(ConversionConfig.ConversionSettings.SolutionExternalVersion) ?
