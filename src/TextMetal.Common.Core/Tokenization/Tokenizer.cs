...
namespace TextMetal.Common.Core.Tokenization
{
	...
	public sealed class Tokenizer
	{
		...
		{
			if (DataType.Instance.IsNullOrWhiteSpace(tokenizedValue))
		...
		private string ReplacementMatcherEx(Match match, IWildcardTokenReplacementStrategy wildcardTokenReplacementStrategy)
		{
			...
			{
				return GetOriginalValueOrThrowExecption(this.StrictMatching, match.Value, string.Format("function exception {{" + Environment.NewLine + "{0}" + Environment.NewLine + "}}", Reflexion.Instance.GetErrors(ex, 0)));
			}
